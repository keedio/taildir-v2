...
public class File implements Comparable {
   ...
   */
    public File moveTo(Directory Destination){
        return moveTo(new File(Destination, getName()), true);
    }
   ...
   *  file is returned.
   */
    public File moveTo(File Destination, boolean Overwrite){
   ...
   */
    public BufferedReader getBufferedReader(String charsetName){
        ...
        if (File.exists()){
            try{
                return new BufferedReader(new InputStreamReader(this.getInputStream(),charsetName));
            }
            catch (Exception e){
            }
        }
