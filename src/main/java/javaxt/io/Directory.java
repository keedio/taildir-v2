...
public class Directory implements Comparable {
   ...
   */
    public Date getDate(){
        if (file!=null && file.exists()) return new Date(file.lastModified());
        File.FileAttributes attr = getFileAttributes();
        if (attr!=null) return attr.getLastWriteTime();
        else{
            java.io.File file = getFile();
            if (file.exists()) return new Date(file.lastModified());
        }
        return null;
    }
   ...
   */
    public File.FileAttributes getFileAttributes(){
        if (attr==null || (new Date().getTime()-attr.lastUpdate)>1000){
            try{
        ...
        if (this.exists()){
            ...
            if (RecursiveSearch){
              ...
              //Spawn threads used to crawl through the file system
                long directoryID = Long.valueOf(Calendar.getInstance().getTimeInMillis() + "" + new Random().nextInt(100000)).longValue();
  ...
  */
    public class Event{
        ...
        public static final int READ_FIRST = 4;
  ...
  //**************************************************************************
    ...
    public final void run(){ 
        ...
        if (!File.loadDLL()){
            ...
            timer.schedule( new EventMonitor(), new Date(), 1000 );
        }
        else{
            try {
